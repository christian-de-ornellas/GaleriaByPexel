...
function Home() {
  ...
  const getImage = async (page) => {
    try {
      const response = await client.photos.curated({page, per_page:20, query: search}); 
  ...
  return (
    <>
      ...
      <c.ContainerGeral>
        ...
        <Button onClick={handleNextpage} text={"Proximo"} width={'250px'} height={'50px'} />
        <Button onClick={handlePreviusPage} text={"Anterior"}  width={'250px'} height={'50px'} />
    </>
  );
}
